...
$workingDirectory = Join-Path $PSScriptRoot '..\storage\official-source'
...
foreach ($file in $files) { Copy-Item -LiteralPath (Join-Path $sourceDirectory $file) -Destination (Join-Path $workingDirectory $file) -Force }
...
try {
  foreach ($file in $files) {
    $source = Join-Path $workingDirectory $file
