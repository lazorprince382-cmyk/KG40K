...
(() => {
  ...
  if(!document.querySelector('link[href*="legal-biodata-styles.css"]')){
    const link=document.createElement("link");link.rel="stylesheet";link.href="/legal-biodata-styles.css?v=34";document.head.appendChild(link);
  }
  ...
  const B=()=>state.legalBio||{records:[],stats:{total:0,verified:0,complete:0,attention:0},access:{}};
  const value=v=>esc(v||" - ");
  ...
  async function loadBio(q="",status="all"){
    state.legalBio=await api(`/api/legal/bio-data?q=${encodeURIComponent(q)}&status=${encodeURIComponent(status)}`);
  ...
  D.views["legal-bio-data"]=()=>{
    ...
    return `<div class="bio-page">
      <div class="bio-protection">${icons.lock}<div><strong>Protected Administration & Records register</strong><span>Bio data is linked automatically to verified membership registration. SACCO balances and loans are not shown here.</span></div><b>LEGAL ACCESS</b></div>
      ...
      <form class="bio-search" data-bio-search><div>${icons.search}<input name="q" value="${esc(b.query||"")}" placeholder="Search name, member number, National ID, phone, occupation, village, district or next of kin..."></div><select name="status"><option value="all">All bio records</option>${["verified","complete","pending","needs_update"].map(x=>option(b.status,x)).join("")}</select><button class="button primary">${icons.search}Search Bio Data</button><button type="button" class="button secondary" data-bio-clear>Clear</button></form>
      <div class="bio-result-head"><div><h2>Member bio-data register</h2><p>${b.records.length} matching protected records</p></div><span>${icons.shield}Access is logged and role controlled</span></div>
      <div class="bio-grid">${b.records.map(profile).join("")||empty("No member bio data matched this search.")}</div>
    </div>`;
  };
  ...
  function detail(record){
    modal("Member bio data",`${record.memberNumber}  -  Protected organization record`,`
      <div class="bio-detail">
        ...
        <section><h3>Personal bio data</h3><dl><div><dt>Date of birth</dt><dd>${date(record.dateOfBirth)}</dd></div><div><dt>Gender</dt><dd>${value(record.gender?.replaceAll("_"," "))}</dd></div><div><dt>Marital status</dt><dd>${value(record.maritalStatus)}</dd></div><div><dt>Nationality</dt><dd>${value(record.nationality)}</dd></div><div><dt>Blood group</dt><dd>${value(record.bloodGroup)}</dd></div><div><dt>Disability information</dt><dd>${value(record.disabilityNotes)}</dd></div></dl></section>
        ...
        <section><h3>Emergency contact & records</h3><dl><div><dt>Contact name</dt><dd>${value(record.emergencyContactName)}</dd></div><div><dt>Contact phone</dt><dd>${value(record.emergencyContactPhone)}</dd></div><div><dt>Relationship</dt><dd>${value(record.emergencyContactRelationship)}</dd></div><div><dt>ID document</dt><dd>${value(record.identityDocumentReference)}</dd></div><div><dt>Profile photo</dt><dd>${value(record.profilePhotoReference)}</dd></div><div><dt>Verified by</dt><dd>${value(record.verifiedBy)}${record.verifiedAt?`  -  ${date(record.verifiedAt,true)}`:""}</dd></div></dl><blockquote>${value(record.recordNotes)}</blockquote></section>
  ...
  async function resetPassword(record){
    ...
    try{const result=await api(`/api/users/${record.userId}/reset-password`,{method:"POST",body:JSON.stringify(temporary?{password:temporary}:{})});closeModal();modal("Temporary password issued",`${record.fullName} - ${record.loginEmail}`,`<div class="bio-password-result"><p>Give this password securely to the member. It is shown only now and must be changed at the next sign-in.</p><code>${esc(result.temporaryPassword)}</code><button class="button primary" data-close-modal>Done</button></div>`);document.querySelectorAll("[data-close-modal]").forEach(x=>x.addEventListener("click",closeModal));await loadBio(B().query||"",B().status||"all");}catch(error){toast(error.message);}
    ...
    const fields=[
      ...
      ["emergencyContactRelationship","Relationship"],["bloodGroup","Blood group"],
      ["profilePhotoReference","Profile photo reference"],["identityDocumentReference","Identity document reference"]
    ];
    modal("Update member bio data",`${record.memberNumber}  -  ${record.fullName}`,`<form class="form bio-edit-form" data-bio-form="${record.memberId}">
      ...
      <div class="form-grid">
        ...
        <div class="field full"><label>Disability or accessibility notes</label><textarea name="disabilityNotes">${esc(record.disabilityNotes||"")}</textarea></div>
        <div class="field full"><label>Administration record notes</label><textarea name="recordNotes">${esc(record.recordNotes||"")}</textarea></div>
  ...
  D.binders.push(cfg=>{
    ...
    document.querySelector("[data-bio-search]")?.addEventListener("submit",async event=>{
      ...
      try{await loadBio(data.q,data.status);render();}catch(error){toast(error.message);}
    });
