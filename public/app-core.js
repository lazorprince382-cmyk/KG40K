...
function headActions() {
  ...
  if(state.role==="Finance Officer") {
    ...
    if(state.page==="finance-bank") return `<div class="head-actions"><button class="button primary" data-finance-modal="transfer-uap">${icons.arrowUp||icons.plus}Transfer to UAP</button><button class="button secondary" data-finance-modal="withdraw-uap">${icons.withdraw}Withdraw from UAP</button><button class="button secondary" data-finance-modal="account">${icons.plus}Add cash account</button></div>`;
    if(state.page==="finance-unit-trust") return `<div class="head-actions"><button class="button primary" data-finance-modal="transfer-uap">${icons.arrowUp||icons.plus}Transfer to UAP</button><button class="button secondary" data-finance-modal="withdraw-uap">${icons.withdraw}Withdraw from UAP</button></div>`;
...
function executiveFinancialWidget(e) {
  ...
  return `<section class="exec-panel exec-financial"><div class="exec-panel-head"><div><h3>Company funds (live)</h3><p>Same live positions as the Finance dashboard</p></div><button data-executive-page="executive-finance">Full summary ></button></div>
    <div class="exec-finance-summary"><span>UAP account<strong>${money(uap)}</strong></span><span>Centenary bank<strong>${money(bank)}</strong></span><span>Money in loans<strong>${money(loans)}</strong></span><span>Total company funds<strong>${money(company)}</strong></span></div>
...
function executiveFinanceSummary(e) {
  ...
  return `<div class="exec-module-metrics executive-finance-metrics">
    ${executiveModuleMetric("UAP account",money(uap),"violet")}
...
function formatLedgerDate(value){
  ...
  if(!Number.isNaN(parsed.getTime()))return formatLedgerDate(parsed);
  return "—";
}
function savingsSplitFromText(text){
  const raw=String(text||"");
  const amount=rawValue=>Number(String(rawValue||"").replace(/,/g,""));
  const savings=raw.match(/(?:^|[^\d])(?:ugx\s*)?([\d,]+(?:\.\d+)?)\s*savings\b/i)
    ||raw.match(/\bsavings\s*(?:=|:)?\s*(?:ugx\s*)?([\d,]+(?:\.\d+)?)/i);
  const received=raw.match(/\breceived\s*(?:=|:)?\s*(?:ugx\s*)?([\d,]+(?:\.\d+)?)/i)
    ||raw.match(/\brecorded\s+(?:ugx\s*)?([\d,]+)/i)
    ||raw.match(/\bdeposit\s+(?:ugx\s*)?([\d,]+)/i);
  const welfare=raw.match(/\b(?:ugx\s*)?([\d,]+(?:\.\d+)?)\s*welfare\b/i)
    ||raw.match(/\bwelfare\s*(?:share|taken)?\s*(?:=|:)?\s*(?:ugx\s*)?([\d,]+(?:\.\d+)?)/i);
  return {savings:amount(savings?.[1]),received:amount(received?.[1]),welfare:amount(welfare?.[1])};
}
...
function financeCanDeleteSavings(f=state.finance){
  return f?.access?.canDeleteSavings!==false&&state.role!=="Auditor"&&state.user?.role!=="Auditor"&&state.role!=="Member"&&state.user?.role!=="Member";
}
function savingsRequestAmount(row){
  const split=savingsSplitFromText(row.notes||row.description||"");
  const stored=Number(row.savingsAmount||row.amount||0);
  const received=split.received||Number(row.amount||0);
  if(split.welfare>0&&stored>=received-0.5){
    const savings=Math.max(0,received-split.welfare);
    return `${money(savings)}<small class="table-sub">Savings · received ${money(received)} · welfare ${money(split.welfare)}</small>`;
  }
  if(split.welfare>0&&stored>0&&stored<received-0.5){
    return `${money(stored)}<small class="table-sub">Savings · received ${money(received)} · welfare ${money(split.welfare)}</small>`;
  }
  return money(row.amount);
}
function financeIncomeAmountCell(row){
  if(!/member\s*savings|savings deposit/i.test(String(row.category||"")))return money(row.amount);
  const split=savingsSplitFromText(row.description||"");
  const received=Number(row.amount||0);
  const welfare=split.welfare>0&&split.welfare<received?split.welfare:0;
  const savings=Number(row.savingsAmount)>0&&Number(row.savingsAmount)<=received
    ?Number(row.savingsAmount)
    :(split.savings>0&&split.savings<=received?split.savings:(welfare?Math.max(0,received-welfare):received));
  if(welfare>0)return `${money(received)}<small class="table-sub">On Centenary · savings ${money(savings)} · welfare ${money(welfare)}</small>`;
  return `${money(received)}<small class="table-sub">On Centenary</small>`;
}
function financeSavingsApprovalQueue(f){
  const rows=(f.savingsRequests||f.pendingSavings||[]).filter(x=>x.status!=="voided");
  if(!rows.length)return "";
  const canApprove=financeCanApproveSavings(f);
  const canDelete=financeCanDeleteSavings(f);
  const waiting=rows.filter(x=>["pending","pending_finance_review"].includes(x.status)).length;
  return `<section class="finance-panel finance-entry-review"><div class="finance-panel-head"><div><h3>Member savings requests</h3><p>Approve to post the full receipt to Centenary. The first deposit in a month sends UGX 25,000 to welfare and records the rest as savings. Approved requests stay here as a record. Delete removes the request from Finance and from the member's requests.</p></div><strong>${waiting} waiting</strong></div>
    <div class="finance-voucher-list">${rows.map(x=>{
      const open=["pending","pending_finance_review"].includes(x.status);
      const label=["completed","verified","approved"].includes(x.status)?"approved":x.status;
      return `<article><div class="finance-voucher-main"><span>${escapeHtml(x.reference)} · Member savings</span><h3>${escapeHtml(x.member||"Member")}</h3><p>${escapeHtml(x.memberNumber||"")} · ${escapeHtml(x.method||"")} · ${escapeHtml(x.paymentReference||"No payment reference")}</p><small>${x.createdAt?new Date(x.createdAt).toLocaleString():""} · ${escapeHtml(x.submittedBy||"Member submission")}</small></div><div class="finance-voucher-value"><strong>${savingsRequestAmount(x)}</strong>${status(label)}</div><div class="finance-voucher-actions">${x.hasEvidence?`<a class="button small secondary" href="/api/transactions/${x.id}/evidence" target="_blank">Receipt</a>`:""}${open&&canApprove?`<button type="button" class="approve" data-finance-savings-review="${x.id}" data-decision="approve">Approve</button><button type="button" class="reject" data-finance-savings-review="${x.id}" data-decision="reject">Reject</button>`:""}${canDelete?`<button type="button" class="reject" data-finance-savings-delete="${x.id}">Delete</button>`:""}</div></article>`;
    }).join("")}</div></section>`;
}
...
function financeWelfareProgressWidget(f){
  ...
  const newRows=(standing.newMembers||[]).slice(0,6).map(m=>`<tr><td><strong>${escapeHtml(m.member)}</strong><small>${escapeHtml(m.memberNumber||"")}${m.isNewMember?" · new":""}</small></td><td>${m.joinedAt?new Date(m.joinedAt).toLocaleDateString("en-GB"):"—"}</td><td>${money(m.collected)}</td><td>${money(m.savingsBalance||0)}</td><td>${escapeHtml(m.sinceLabel||"—")}</td></tr>`).join("")
    ||`<tr><td colspan="5"><div class="member-empty">No new-member welfare rows yet.</div></td></tr>`;
  return `<section class="finance-panel finance-subscription-progress">
...
async function reviewFinanceSavings(id,decision){
  ...
  if(decision==="reject"){
    ...
    comment=String(comment).trim()||"Rejected";
  }else if(!await confirmDialog("Approve this savings submission? The full amount posts to Centenary. The first deposit in the month sends UGX 25,000 to welfare and records the rest as savings."))return;
  try{
  ...
  }catch(error){toast(error.message||"Could not update this savings request.");}
}
async function deleteFinanceSavings(id){
  if(!financeCanDeleteSavings())return toast("You cannot delete this request.");
  if(!await confirmDialog("Delete this request? It will be removed from Finance approvals and from the member's requests. If it was already approved, the Centenary receipt, savings and any welfare taken from that receipt will be reversed."))return;
  try{
    const result=await api(`/api/finance/savings/${id}`,{method:"DELETE"});
    closeModal();
    if(state.page?.startsWith("credits"))await refreshCredits();
    if(state.finance)state.finance=await api("/api/finance/command-center");
    render();
...
function financeMonthSortKey(row){
  if(row.monthKey)return String(row.monthKey);
  const map={Jan:"01",Feb:"02",Mar:"03",Apr:"04",May:"05",Jun:"06",Jul:"07",Aug:"08",Sep:"09",Oct:"10",Nov:"11",Dec:"12"};
  const label=String(row.month||"").slice(0,3);
  return map[label]||"99";
}
function financeShortText(value,limit=72){
  const text=String(value||"").replace(/\s+/g," ").trim();
  if(!text)return "—";
  return text.length>limit?`${escapeHtml(text.slice(0,limit-1))}…`:`${escapeHtml(text)}`;
}
...
function financeCashPositionWidget(f) {
  const c=f.cashPosition||{},available=Number(c.bankBalance||0)+Number(c.cashBalance||0)+Number(c.pettyCash||0);
  const liquidityTotal=available+Number(c.restrictedFunds||0),liquidity=liquidityTotal?Math.round(Number(c.availableFunds||0)/liquidityTotal*100):0;
  const items=[
    ["Centenary bank",c.bankBalance],
    ["UAP account",c.uapBalance||0],
    ["Money in loans",c.loansOutstanding||0],
...
function financeIncomeView() {
  ...
  return `<div class="exec-module-metrics">${executiveModuleMetric("Today",money(f.stats.receiptsToday??f.stats.incomeToday),"green")}${executiveModuleMetric("This month",money(f.stats.monthlyReceipts??f.stats.monthlyIncome),"blue")}${executiveModuleMetric("Receipts",rows.length,"violet")}${executiveModuleMetric("Top source",topSource?topSource.label:"—","orange")}</div>
    <p class="exec-report-preview-note">Every receipt posts to the Centenary bank account. The first deposit in a month takes UGX 25,000 as welfare and the rest as savings — both stay on the same Centenary account. Dates show as DD/MM/YYYY. Deleting a receipt takes that amount back off Centenary.</p>
    ${financeIncomeWidget(f)}${financeDataTable("Income ledger",["Receipt","Date","Payer / Organization","Category","Method","Amount","Status",...(canDelete?["Actions"]:[])],rows.map(x=>[x.receiptNumber||x.reference,formatLedgerDate(x.transactionDate),x.counterparty||"?",x.category,x.paymentMethod||"?",financeIncomeAmountCell(x),status(x.status),...(canDelete?[`<button type="button" class="danger-action" data-finance-income-delete="${x.id}">Delete</button>`]:[])]))}`;
}
...
function financeReceiptsView() {
  ...
  return financeDataTable("Issued organization receipts",["Receipt number","Date","Received from","Category","Method","Amount","Actions"],rows.map(x=>[x.receiptNumber||x.reference,formatLedgerDate(x.transactionDate),x.counterparty||"?",x.category,x.paymentMethod||"?",money(x.amount),`<button class="button small secondary" data-finance-receipt="${x.id}">Download receipt</button>`]));
}
...
function financeBankView() {
  ...
  const welfareOnBank=Number(f.cashPosition?.welfareOnCentenaryThisMonth||f.welfareProgress?.collected||0);
  ...
  <p class="exec-report-preview-note">Welfare is not a second bank. When a member pays UGX 425,000, the whole UGX 425,000 goes onto Centenary. UGX 25,000 is marked as welfare and UGX 400,000 as savings — both already sit in this Centenary balance. Deleting the receipt reverses the same full amount.</p>
...
function financeAnalyticsView() {
  const f=state.finance;
  return `<div class="exec-analytics-grid">${financeRevenueGraph(f)}${financeIncomeWidget(f)}${financeExpenseWidget(f)}${financeForecastWidget(f)}</div>`;
}
...
function configureFinanceIncomeAccounts(form) {
  ...
  const refresh=()=>{
    ...
    if(savings){
      ...
      form.querySelector("[data-account-help]").textContent="Always the Centenary bank account. The first deposit this month sends UGX 25,000 to welfare; later deposits this month are not deducted. The full receipt amount is posted to Centenary immediately — no approval.";
      return;
    }
...
async function submitFinanceForm(event) {
  ...
  if(type==="transfer-uap"||type==="withdraw-uap"){
    ...
    try{
      ...
      const result=await api(type==="withdraw-uap"?"/api/finance/withdraw-from-uap":"/api/finance/transfer-to-uap",{method:"POST",body:JSON.stringify(data)});
      closeModal();
      state.finance=await api("/api/finance/command-center");
      state.unitTrust=await loadFinanceUnitTrust(state.unitTrust?.month||defaultUnitTrustMonth(state.unitTrust?.availableMonths));
      render();
      toast(type==="withdraw-uap"
        ?`Withdrew ${money(result.amount)} from UAP. Unit Trust balance ${money(result.uapBalance)}.`
        :`Transferred ${money(result.amount)} to UAP. Unit Trust balance ${money(result.uapBalance)}.`);
...
async function deleteFinanceIncome(id){
  ...
  const message=savings
    ?`Delete receipt ${label} for ${money(entry.amount)}? Centenary will be reduced by this full receipt amount. Member savings and any welfare taken from this receipt will also be reversed.`
...
function creditsTransactionDetails(id) {
  ...
  const actions=savingsPending&&canApproveSavings
    ?`<button type="button" class="button danger" data-finance-savings-review="${t.id}" data-decision="reject">Reject</button><button type="button" class="button danger" data-finance-savings-delete="${t.id}">Delete</button><button type="button" class="button primary" data-finance-savings-review="${t.id}" data-decision="approve">Approve and post to Centenary</button>`
    :pending&&canVerify
  ...
  document.querySelectorAll("[data-finance-savings-review]").forEach(button=>button.onclick=event=>{event.preventDefault();reviewFinanceSavings(t.id,button.dataset.decision);});
  document.querySelectorAll("[data-finance-savings-delete]").forEach(button=>button.onclick=event=>{event.preventDefault();deleteFinanceSavings(t.id);});
}
...
function bind() {
  ...
  document.querySelectorAll("[data-finance-savings-review]").forEach(el=>el.addEventListener("click",event=>{
    event.preventDefault();
    reviewFinanceSavings(el.dataset.financeSavingsReview,el.dataset.decision);
  }));
  document.querySelectorAll("[data-finance-savings-delete]").forEach(el=>el.addEventListener("click",event=>{
    event.preventDefault();
    deleteFinanceSavings(el.dataset.financeSavingsDelete);
  }));
  ...
  document.querySelectorAll("[data-deposit-decision-row]").forEach(el=>el.addEventListener("click",()=>{
    const id=el.dataset.depositDecisionRow;
    const tx=state.credits?.transactions?.find(item=>String(item.id)===String(id));
    decideCreditsDeposit(id,el.dataset.decision,tx?.type==="Loan repayment");
  }));
