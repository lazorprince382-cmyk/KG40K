...
(() => {
  ...
  const targetLine=(label,paid,target)=>{const percent=target?Math.min(100,Math.round(Number(paid)/Number(target)*100)):0,variance=Number(paid)-Number(target);return `<div class="member-target-line"><div><strong>${label}</strong><span>${money(paid)} paid of ${money(target)}</span></div><b>${percent}%</b><i><em style="width:${percent}%"></em></i><small class="${variance<0?"behind":"ahead"}">${variance<0?`Short by ${money(Math.abs(variance))}`:`Ahead by ${money(variance)}`}</small></div>`;};
  function financialYearPanel(){const f=C()?.financialYearProgress;if(!f)return "";return panel(`${f.fiscalYear} contribution progress`,`${f.monthsDue} completed month${Number(f.monthsDue)===1?"":"s"} due since 1 July  -  the current month counts only after it ends`,`${targetLine("Savings for completed months",f.savingsPaid,f.expectedSavingsToDate)}${targetLine("Annual share contribution",f.sharePaid,f.annualShareTarget)}${targetLine("Annual subscription fee",f.subscriptionPaid,f.annualSubscriptionFee)}<div class="member-policy-note"><b>Monthly savings:</b> ${money(f.monthlySavingsTarget)}  -  <b>Full-year savings target:</b> ${money(f.annualSavingsTarget)}</div>`);}
