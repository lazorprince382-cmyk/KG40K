...
(() => {
  ...
  function financialYearPanel(){const f=C()?.financialYearProgress;if(!f)return "";return panel(`${f.fiscalYear} contribution progress`,`${f.monthsDue} month${Number(f.monthsDue)===1?"":"s"} due since 1 July  -  balances update only after verification`,`${targetLine("Savings expected by today",f.savingsPaid,f.expectedSavingsToDate)}${targetLine("Annual share contribution",f.sharePaid,f.annualShareTarget)}${targetLine("Annual subscription fee",f.subscriptionPaid,f.annualSubscriptionFee)}<div class="member-policy-note"><b>Monthly savings:</b> ${money(f.monthlySavingsTarget)}  -  <b>Full-year savings target:</b> ${money(f.annualSavingsTarget)}</div>`);}
