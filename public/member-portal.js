...
(() => {
  ...
  function welfare(){
    const w=C().welfare||{},s=C().summary||{},fund=C().welfareFund||{},org=C()?.orgWelfareStanding||{};
    const since=Number(org.collectedSince||fund.collectedSince||s.welfareContributedSince||0);
    const assistance=Number(org.assistancePaid||fund.assistancePaid||0);
    const currentRaw=Number(org.currentBalance??fund.currentBalance);
    const current=Number.isFinite(currentRaw)?Math.max(0,currentRaw):Math.max(0,since-assistance);
  ...
  async function submitMemberRepayment(event){
    ...
    if(file.size>100*1024*1024)return fail("The receipt is larger than 100 MB.");
  ...
  async function submitMemberDeposit(event){
    ...
    if(file.size>100*1024*1024)return fail("The receipt is larger than 100 MB. Choose a smaller photo or PDF.",form.elements.receipt);
