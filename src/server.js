...
/** Calendar YYYY-MM-DD in Africa/Kampala (avoids UTC date-shift on DATE values). */
const kampalaDateKey = (value) => {
...
const kampalaTodayKey = () => new Date().toLocaleDateString("en-CA", { timeZone: "Africa/Kampala" });
const isPastDueDate = (dueValue, todayKey = kampalaTodayKey()) => {
  const dueKey = kampalaDateKey(dueValue);
...
app.get("/api/executive/command-center",auth,requireExecutive("view"),asyncRoute(async(req,res)=>{
  ...
  const [memberStats,departments,approvals,financeTotals,savings,loanStats,investments,welfare,legal,auditIssues,supervisory,meetings,
    ...
    query(`SELECT COALESCE(SUM(amount) FILTER (WHERE entry_type='income' AND status IN ('approved','completed')
      ...
      COALESCE(SUM(amount) FILTER (WHERE entry_type='expense' AND status IN ('approved','completed')
        AND COALESCE(payment_method,'') NOT ILIKE 'Management accounts import'
        AND transaction_date>=date_trunc('month',CURRENT_DATE)::date),0)::float AS expenditure_month,
    ...
    query(`SELECT f.id,f.reference,f.entry_type AS "entryType",f.category,f.description,f.amount::float,f.status,
      f.payment_method AS "paymentMethod",f.created_at AS "createdAt",d.name AS department
      FROM organization_finance_entries f JOIN departments d ON d.id=f.department_id
      WHERE COALESCE(f.payment_method,'') NOT ILIKE 'Management accounts import'
  ...
  // Live FY uses operational ledger only — do not fold prior-year statement totals into today's command center.
  ...
  res.json({
    ...
    stats:{totalMembers:memberStats.rows[0].total,activeMembers:memberStats.rows[0].active,newMembers:memberStats.rows[0].new_this_month,
      ...
      activeInvestments:investment.running,welfareFundBalance:welfareBalance,
...
app.get("/api/executive/projects/:id",auth,requireExecutive("view"),asyncRoute(async(req,res)=>{
  ...
  project.budgetUtilization=project.isUnitTrust
    ? (Number(project.budget)?Number((Number(project.capitalRaised||project.budget)/Number(project.budget)*100).toFixed(1)):100)
    : (Number(project.budget)?Number((Number(project.expenses)/Number(project.budget)*100).toFixed(1)):0);
...
app.post("/api/finance/transfer-to-uap",auth,requireFinance("create"),asyncRoute(async(req,res)=>{
  const amount=Number(req.body.amount),fromAccountId=Number(req.body.fromAccountId||req.body.accountId);
  const transferDate=String(req.body.date||new Date().toISOString().slice(0,10));
  const note=String(req.body.note||req.body.description||"").trim();
  if(!Number.isFinite(amount)||amount<=0)return res.status(400).json({error:"Enter a positive transfer amount"});
  if(!Number.isInteger(fromAccountId))return res.status(400).json({error:"Choose the company bank account sending money to UAP"});
  if(!/^\d{4}-\d{2}-\d{2}$/.test(transferDate))return res.status(400).json({error:"Use a valid transfer date"});
  const result=await transaction(async client=>{
    const source=(await client.query(`SELECT * FROM finance_accounts WHERE id=$1 AND active=true FOR UPDATE`,[fromAccountId])).rows[0];
    if(!source){const error=new Error("Source bank account not found");error.status=404;throw error;}
    if(source.account_code==="GL-4500"){const error=new Error("Choose Centenary or another bank account, not the UAP account");error.status=400;throw error;}
    if(Number(source.balance)<amount-0.005){const error=new Error("Source account does not have enough balance for this transfer");error.status=400;throw error;}
    let uap=(await client.query(`SELECT * FROM finance_accounts WHERE account_code='GL-4500' AND active=true FOR UPDATE`)).rows[0];
    if(!uap){
      uap=(await client.query(`INSERT INTO finance_accounts
        (account_code,account_name,account_type,bank_name,account_number,balance,opening_balance,opening_balance_date,
         notes,restricted,active,created_by,updated_at)
        VALUES ('GL-4500','Old Mutual Unit Trust (UAP)','restricted','Old Mutual Investment Group','99171-CKA1073440',0,0,$1,
          'Unit Trust / Old Mutual umbrella fund',true,true,$2,NOW()) RETURNING *`,[transferDate,req.user.id])).rows[0];
    }
    const financeDepartment=(await client.query(`SELECT id FROM departments WHERE code='finance'`)).rows[0];
    const outRef=reference("FIN-UAP-OUT"),inRef=reference("FIN-UAP-IN");
    const outEntry=(await client.query(`INSERT INTO organization_finance_entries
      (department_id,reference,entry_type,category,description,counterparty,payment_method,amount,status,receipt_number,
        transaction_date,recorded_by,approved_by,approved_at,finance_account_id)
      VALUES ($1,$2,'transfer','Internal transfer to UAP',$3,$4,'Bank transfer',$5,'completed',$6,$7,$8,$8,NOW(),$9)
      RETURNING id,reference`,[financeDepartment.id,outRef,
      note||`Internal move of company bank funds (member deposits surplus) to Old Mutual Unit Trust (UAP) — not organization expense/income`,uap.account_name,amount,receiptReference("UAP"),transferDate,req.user.id,source.id])).rows[0];
    const inEntry=(await client.query(`INSERT INTO organization_finance_entries
      (department_id,reference,entry_type,category,description,counterparty,payment_method,amount,status,receipt_number,
        transaction_date,recorded_by,approved_by,approved_at,finance_account_id)
      VALUES ($1,$2,'transfer','Internal transfer from company bank',$3,$4,'Bank transfer',$5,'completed',$6,$7,$8,$8,NOW(),$9)
      RETURNING id,reference`,[financeDepartment.id,inRef,
      note||`UAP received internal transfer from ${source.account_name} — already held as company/member funds, not new outside income`,source.account_name,amount,receiptReference("UAPIN"),transferDate,req.user.id,uap.id])).rows[0];
    await client.query(`UPDATE finance_accounts SET balance=balance-$1,updated_at=NOW() WHERE id=$2`,[amount,source.id]);
    const uapUpdated=(await client.query(`UPDATE finance_accounts SET balance=balance+$1,updated_at=NOW() WHERE id=$2 RETURNING balance::float AS balance`,[amount,uap.id])).rows[0];
    await client.query(`INSERT INTO unit_trust_movements
      (movement_date,description,deposit_amount,interest_amount,withdrawal_amount,balance_after,source_reference,finance_entry_id,created_by)
      VALUES ($1,'Transfer from Centenary (company bank / member deposits)',$2,0,0,$3,$4,$5,$6)`,
      [transferDate,amount,uapUpdated.balance,outRef,inEntry.id,req.user.id]);
    await client.query(`INSERT INTO settings (key,value) VALUES ('organizationUapBalance',$1)
      ON CONFLICT (key) DO UPDATE SET value=EXCLUDED.value,updated_at=NOW()`,[String(uapUpdated.balance)]);
    await client.query(`UPDATE investment_fund_accounts SET current_value=$1,updated_at=NOW()
      WHERE reference IN ('FUND-OLD-MUTUAL-2025','FUND-UAP-UMBRELLA')`,[uapUpdated.balance]);
    return {outRef,inRef,uapBalance:uapUpdated.balance,fromBalance:Number(source.balance)-amount,amount};
  });
  await audit({userId:req.user.id,action:"FINANCE_TRANSFER_TO_UAP",entityType:"finance_account",
    details:`UGX ${amount} → UAP (${result.outRef} / ${result.inRef})`,...metadata(req)});
  res.status(201).json(result);
}));
app.post("/api/finance/withdraw-from-uap",auth,requireFinance("create"),asyncRoute(async(req,res)=>{
  const amount=Number(req.body.amount),toAccountId=Number(req.body.toAccountId||req.body.accountId);
  const transferDate=String(req.body.date||new Date().toISOString().slice(0,10));
  const note=String(req.body.note||req.body.description||"").trim();
  if(!Number.isFinite(amount)||amount<=0)return res.status(400).json({error:"Enter a positive withdrawal amount"});
  if(!Number.isInteger(toAccountId))return res.status(400).json({error:"Choose the company bank account receiving money from UAP"});
  if(!/^\d{4}-\d{2}-\d{2}$/.test(transferDate))return res.status(400).json({error:"Use a valid withdrawal date"});
  const result=await transaction(async client=>{
    const destination=(await client.query(`SELECT * FROM finance_accounts WHERE id=$1 AND active=true FOR UPDATE`,[toAccountId])).rows[0];
    if(!destination){const error=new Error("Destination bank account not found");error.status=404;throw error;}
    if(destination.account_code==="GL-4500"){const error=new Error("Choose Centenary or another bank account, not the UAP account");error.status=400;throw error;}
    const uap=(await client.query(`SELECT * FROM finance_accounts WHERE account_code='GL-4500' AND active=true FOR UPDATE`)).rows[0];
    if(!uap){const error=new Error("UAP account not found");error.status=404;throw error;}
    if(Number(uap.balance)<amount-0.005){const error=new Error("UAP account does not have enough balance for this withdrawal");error.status=400;throw error;}
    const financeDepartment=(await client.query(`SELECT id FROM departments WHERE code='finance'`)).rows[0];
    const outRef=reference("FIN-UAP-WD"),inRef=reference("FIN-UAP-BANK");
    const outEntry=(await client.query(`INSERT INTO organization_finance_entries
      (department_id,reference,entry_type,category,description,counterparty,payment_method,amount,status,receipt_number,
        transaction_date,recorded_by,approved_by,approved_at,finance_account_id)
      VALUES ($1,$2,'transfer','Internal withdrawal from UAP',$3,$4,'Bank transfer',$5,'completed',$6,$7,$8,$8,NOW(),$9)
      RETURNING id,reference`,[financeDepartment.id,outRef,
      note||`Internal withdrawal from Old Mutual Unit Trust (UAP) to company bank — not organization expense`,destination.account_name,amount,receiptReference("UAPWD"),transferDate,req.user.id,uap.id])).rows[0];
    await client.query(`INSERT INTO organization_finance_entries
      (department_id,reference,entry_type,category,description,counterparty,payment_method,amount,status,receipt_number,
        transaction_date,recorded_by,approved_by,approved_at,finance_account_id)
      VALUES ($1,$2,'transfer','Internal transfer from UAP',$3,$4,'Bank transfer',$5,'completed',$6,$7,$8,$8,NOW(),$9)`,
      [financeDepartment.id,inRef,
      note||`Company bank received internal withdrawal from UAP — already held as company funds`,uap.account_name,amount,receiptReference("UAPBK"),transferDate,req.user.id,destination.id]);
    const uapUpdated=(await client.query(`UPDATE finance_accounts SET balance=balance-$1,updated_at=NOW() WHERE id=$2 RETURNING balance::float AS balance`,[amount,uap.id])).rows[0];
    await client.query(`UPDATE finance_accounts SET balance=balance+$1,updated_at=NOW() WHERE id=$2`,[amount,destination.id]);
    await client.query(`INSERT INTO unit_trust_movements
      (movement_date,description,deposit_amount,interest_amount,withdrawal_amount,balance_after,source_reference,finance_entry_id,created_by)
      VALUES ($1,'Withdrawal to company bank (Centenary)',0,0,$2,$3,$4,$5,$6)`,
      [transferDate,amount,uapUpdated.balance,outRef,outEntry.id,req.user.id]);
    await client.query(`INSERT INTO settings (key,value) VALUES ('organizationUapBalance',$1)
      ON CONFLICT (key) DO UPDATE SET value=EXCLUDED.value,updated_at=NOW()`,[String(uapUpdated.balance)]);
    await client.query(`UPDATE investment_fund_accounts SET current_value=$1,updated_at=NOW()
      WHERE reference IN ('FUND-OLD-MUTUAL-2025','FUND-UAP-UMBRELLA')`,[uapUpdated.balance]);
    return {outRef,inRef,uapBalance:uapUpdated.balance,toBalance:Number(destination.balance)+amount,amount};
  });
  await audit({userId:req.user.id,action:"FINANCE_WITHDRAW_FROM_UAP",entityType:"finance_account",
    details:`UGX ${amount} ← UAP (${result.outRef} / ${result.inRef})`,...metadata(req)});
  res.status(201).json(result);
}));
app.get("/api/finance/unit-trust",auth,requireFinance("view"),asyncRoute(async(req,res)=>{
  ...
  const ordered=monthStart?movements:[...movements].reverse();
  let opening=Number(account?.balance||0);
  if(monthStart){
    const prior=await one(`SELECT balance_after::float AS balance FROM unit_trust_movements
      WHERE movement_date<$1::date ORDER BY movement_date DESC,id DESC LIMIT 1`,[monthStart]);
    if(prior)opening=Number(prior.balance);
    else if(ordered[0]?.description==="Opening Balance")opening=Number(ordered[0].balance);
    else if(ordered[0])opening=Number(ordered[0].balance)-Number(ordered[0].deposit||0)-Number(ordered[0].interest||0)+Number(ordered[0].withdrawal||0);
  }else if(ordered[0]?.description==="Opening Balance")opening=Number(ordered[0].balance);
  else if(ordered[0])opening=Number(ordered[0].balance)-Number(ordered[0].deposit||0)-Number(ordered[0].interest||0)+Number(ordered[0].withdrawal||0);
  const closing=ordered.length?Number(ordered[ordered.length-1].balance):Number(account?.balance||opening);
...
app.get("/api/finance/unit-trust/export.csv",auth,requireFinance("view"),asyncRoute(async(req,res)=>{
  ...
  const lines=[header.map(esc).join(","),...movements.map(r=>[
    r.date?new Date(r.date).toISOString().slice(0,10):"",r.description,r.deposit||0,r.interest||0,
    r.withdrawal||0,r.rate??"",r.balance||0,r.sourceReference||""
  ].map(esc).join(","))];
  const stamp=monthStart?month:"all";
...
app.delete("/api/finance/unit-trust/movements/:id",auth,requireFinance("edit"),asyncRoute(async(req,res)=>{
  ...
  const row=await one(`SELECT id,movement_date AS "date",description,deposit_amount::float AS deposit,
  ...
  const [members,transactionsResult,loansResult,guarantors,recovery,charges,documents,summary,depositSummary,portfolioSummary,guarantorSummary]=await Promise.all([
    ...
    query(`SELECT l.id,l.reference,m.id AS "memberId",m.member_number AS "memberNumber",m.full_name AS member,
      ...
      COALESCE((SELECT MIN(s.due_date) FROM loan_repayment_schedule s
        WHERE s.loan_id=l.id AND s.status IN ('due','partial','upcoming','overdue') AND s.paid_amount<s.total_due),l.due_date) AS "nextDueDate",
      (
        ...
        + COALESCE((SELECT ROUND(GREATEST(0,s.principal)*p.late_penalty_rate/100,2)::float
          FROM loan_repayment_schedule s
...
async function applyLoanRepayment(client,loanId,amount,options={}){
  ...
  let payingCurrentInstallment=true;
  for(const schedule of schedules){
    ...
    const principalDue=Math.max(0,Number(schedule.principal)-Number(schedule.principal_paid||0));
    if(payingCurrentInstallment){
      interestPart=Math.min(remaining,interestDue);
      remaining-=interestPart;
      principalPart=Math.min(remaining,principalDue);
      remaining-=principalPart;
      payingCurrentInstallment=false;
    }else{
      // Surplus after the current installment reduces the next month's principal first.
      principalPart=Math.min(remaining,principalDue);
      remaining-=principalPart;
      interestPart=Math.min(remaining,interestDue);
      remaining-=interestPart;
    }
    interestApplied+=interestPart; principalApplied+=principalPart;
...
app.post("/api/loans",auth,upload.array("supportingDocument",10),(req,res,next)=>{
  ...
  // When the borrower's 75% savings security fully covers the loan, guarantors are not required.
  const guarantorIds=savingsSecurity&&remainingToCover>0?[...new Set([...(Array.isArray(b.guarantorIds)?b.guarantorIds:[]),b.guarantorId].map(Number).filter(Boolean))]:[];
  ...
  const row=await transaction(async client=>{
    ...
    for(let i=0;i<supportingFiles.length;i++){
      ...
      await client.query(
        `INSERT INTO loan_supporting_documents (loan_id,stored_name,original_name,mime_type,sort_order)
         VALUES ($1,$2,$3,$4,$5)`,
        [created.id,file.filename,file.originalname||file.filename,file.mimetype||null,i+1]
      );
    }
    ...
    const nextNote=collateralSecurity
      ...
      :(pledges.length
        ...
        :`Savings security: borrower 75% fully covers UGX ${borrowerCover.toLocaleString()} — no guarantors required. Processing fee ${processingFee.toLocaleString()} (2%) deducted at disbursement.`);
  ...
  res.status(201).json({id:row.id,reference:ref,status:initialStatus,maximumEligible:policyMaximum,processingFee,borrowerCover,remainingToCover,guarantorsRequired:pledges.length,supportingDocuments:supportingFiles.length});
...
app.post("/api/loans/:id/finance-verification",auth,(_req,res)=>res.status(410).json({error:"Finance does not approve loans. Credit Committee approval is sent directly to Executive."}));async function createRepaymentSchedule(client,loan,amount) {
  ...
  // Equal total payments (amortizing EMI) with reducing-balance monthly interest.
  const monthlyRate=Number(product.annual_rate)/1200,n=Number(loan.term_months),round=value=>Math.round((value+Number.EPSILON)*100)/100;
  let payment;
  if(monthlyRate===0)payment=round(amount/n);
  else{
    const factor=Math.pow(1+monthlyRate,n);
    payment=round(amount*monthlyRate*factor/(factor-1));
  }
  ...
  for(let installment=1;installment<=n;installment++){
    ...
    let principal=installment===n?openingBalance:round(payment-interest);
...
app.post("/api/loans/:id/disburse",auth,asyncRoute(async(req,res)=>{
  ...
  const loan=await one("SELECT id,member_id,supporting_document_stored_name AS stored,supporting_document_original_name AS original,supporting_document_mime_type AS mime FROM loans WHERE id=$1",[req.params.id]);
  if(!loan)return res.status(404).json({error:"Loan not found"});
  ...
  const docId=Number(req.query.docId||req.query.id||0);
  const index=Math.max(1,Number(req.query.index||1));
  let doc=null;
  if(docId){
    doc=await one(`SELECT stored_name AS stored,original_name AS original,mime_type AS mime
      FROM loan_supporting_documents WHERE loan_id=$1 AND id=$2`,[loan.id,docId]);
  }else{
    doc=await one(`SELECT stored_name AS stored,original_name AS original,mime_type AS mime
      FROM loan_supporting_documents WHERE loan_id=$1 ORDER BY sort_order,id
      OFFSET $2 LIMIT 1`,[loan.id,index-1]);
  }
  if(!doc&&index===1&&loan.stored)doc={stored:loan.stored,original:loan.original,mime:loan.mime};
  if(!doc?.stored)return res.status(404).json({error:"Loan supporting document not found"});
  const storedName=path.basename(doc.stored);
  const seedDir=path.join(projectRoot,"storage","seed-supporting");
  const candidates=[
    path.join(uploadsDir,storedName),
    path.join(seedDir,storedName),
    storedName.startsWith("seed-")?path.join(seedDir,storedName.slice(5)):null
  ].filter(Boolean);
  let file=candidates.find(candidate=>fs.existsSync(candidate));
  if(!file)return res.status(404).json({error:"Stored loan document is missing"});
  const uploadTarget=path.join(uploadsDir,storedName);
  ...
  const loan=await one(`SELECT l.*,m.full_name AS member,m.member_number,
    CASE WHEN NULLIF(l.custom_product_name,'') IS NOT NULL THEN p.name||' ('||l.custom_product_name||')' ELSE p.name END AS product,
    p.annual_rate,p.late_penalty_rate::float AS late_penalty_rate,l.custom_product_name,l.processing_fee
    FROM loans l JOIN members m ON m.id=l.member_id JOIN loan_products p ON p.id=l.product_id WHERE l.id=$1`,[loanId]);
  if(!loan)return res.status(404).json({error:"Loan not found"});
  ...
  const [guarantors,events,schedule,repayments,disbursement,creditsProgress,executiveProgress,supportingDocuments]=await Promise.all([
    ...
    query(`SELECT s.id AS "scheduleId",s.installment_number AS installment,s.due_date AS "dueDate",
      ...
      s.principal_paid::float AS "principalPaid",s.interest_paid::float AS "interestPaid",s.status,
      CASE
        WHEN s.status='paid' THEN 0
        WHEN s.due_date < (CURRENT_TIMESTAMP AT TIME ZONE 'Africa/Kampala')::date THEN
          COALESCE(NULLIF((SELECT ROUND(c.amount::numeric,2) FROM loan_charges c
            WHERE c.schedule_id=s.id AND c.charge_type='Late payment penalty'
              AND c.status IN ('outstanding','partial','settled')
            ORDER BY c.id DESC LIMIT 1),0),
            ROUND((s.principal*(SELECT p.late_penalty_rate FROM loans lx JOIN loan_products p ON p.id=lx.product_id WHERE lx.id=s.loan_id)/100.0)::numeric,2))
        ELSE 0
      END::float AS "penalty",
      CASE
        WHEN s.status='paid' THEN 0
        WHEN s.due_date < (CURRENT_TIMESTAMP AT TIME ZONE 'Africa/Kampala')::date THEN
          COALESCE((SELECT ROUND(GREATEST(0,c.amount-c.paid_amount)::numeric,2) FROM loan_charges c
            WHERE c.schedule_id=s.id AND c.charge_type='Late payment penalty'
              AND c.status IN ('outstanding','partial')
            ORDER BY c.id DESC LIMIT 1),
            ROUND((s.principal*(SELECT p.late_penalty_rate FROM loans lx JOIN loan_products p ON p.id=lx.product_id WHERE lx.id=s.loan_id)/100.0)::numeric,2))
        ELSE 0
      END::float AS "penaltyOutstanding"
      FROM loan_repayment_schedule s WHERE s.loan_id=$1 ORDER BY s.installment_number`,[loan.id]),
    query(`SELECT amount::float, created_at AS "createdAt"
      FROM transactions
      WHERE loan_id=$1 AND type='Loan repayment' AND status='completed'
      ORDER BY created_at,id`,[loan.id]),
    ...
    query(`SELECT id,original_name AS "originalName",mime_type AS "mimeType",sort_order AS "sortOrder"
      FROM loan_supporting_documents WHERE loan_id=$1 ORDER BY sort_order,id`,[loan.id])
  ]);
  let docs=supportingDocuments.rows.map(doc=>({
    ...doc,
    url:`/api/loans/${loan.id}/supporting-document?docId=${doc.id}`
  }));
  if(!docs.length&&loan.supporting_document_stored_name){
    docs=[{id:null,originalName:loan.supporting_document_original_name||"Supporting document",mimeType:loan.supporting_document_mime_type,sortOrder:1,url:`/api/loans/${loan.id}/supporting-document`}];
  }
  const todayKey=kampalaTodayKey();
  const repaymentAmounts=(repayments.rows||[]).map(tx=>Number(tx.amount||0));
  let repaymentCursor=0;
  const scheduleRows=schedule.rows.map((row)=>{
    const isPaid=row.status==="paid";
    // Penalty only after due date, and never on a fully paid installment.
    const penaltyActive=!isPaid&&isPastDueDate(row.dueDate,todayKey);
    const principal=Number(row.principal||0);
    const interest=Number(row.interest||0);
    const emi=roundMoney(principal+interest);
    const rate=Number(loan.late_penalty_rate||5);
    const assessed=roundMoney(principal*(rate/100));
    const rawPenalty=penaltyActive?(Number(row.penalty||0)||assessed):0;
    const rawOutstanding=penaltyActive?(Number(row.penaltyOutstanding||0)||assessed):0;
    const schedulePaid=Number(row.paidAmount||0);
    let amountPaid=schedulePaid;
    let surplusPaid=0;
    if(schedulePaid>0.005){
      if(isPaid&&repaymentCursor<repaymentAmounts.length){
        amountPaid=roundMoney(repaymentAmounts[repaymentCursor]);
        surplusPaid=roundMoney(Math.max(0,amountPaid-emi));
        repaymentCursor+=1;
      }else{
        amountPaid=roundMoney(schedulePaid);
      }
    }
    ...
    const outstanding=isPaid?0:roundMoney(remainingPrincipal+remainingInterest+rawPenalty);
    const remaining=Math.max(0,Number(row.totalDue||0)-schedulePaid);
    const totalDueWithPenalty=roundMoney(emi+rawPenalty);
    const amountDue=roundMoney(remaining+rawOutstanding);
    return {
      ...row,
      dueDate:kampalaDateKey(row.dueDate)||row.dueDate,
      emiTotal:emi,
      penalty:rawPenalty,
      penaltyOutstanding:rawOutstanding,
      totalDueWithPenalty,
      outstanding,
      amountDue,
      amountPaid,
  ...
  res.json({loan,guarantors:guarantors.rows,events:events.rows,schedule:scheduleRows,disbursement,
...
async function runScheduledMaintenance() {
  ...
  await transaction(async client=>{
    ...
    await client.query(`UPDATE loan_repayment_schedule SET status='due'
      WHERE status='upcoming' AND due_date<=(CURRENT_TIMESTAMP AT TIME ZONE 'Africa/Kampala')::date AND paid_amount<total_due`);
    await client.query(`UPDATE loan_repayment_schedule SET status='overdue'
      WHERE status IN ('due','partial') AND due_date < (CURRENT_TIMESTAMP AT TIME ZONE 'Africa/Kampala')::date AND paid_amount<total_due`);
    ...
    // 5% penalty once per overdue installment (day after due date) — not every later month.
    await client.query(`INSERT INTO loan_charges (loan_id,charge_type,amount,status,reason,schedule_id,penalty_period)
      SELECT s.loan_id,'Late payment penalty',ROUND(s.principal*p.late_penalty_rate/100,2),'outstanding',
        'Automatic 5% penalty on the principal due after the due date',s.id,s.due_date
      FROM loan_repayment_schedule s JOIN loans l ON l.id=s.loan_id JOIN loan_products p ON p.id=l.product_id
      WHERE s.status='overdue' AND s.due_date < (CURRENT_TIMESTAMP AT TIME ZONE 'Africa/Kampala')::date
        AND s.paid_amount<s.total_due
        AND NOT EXISTS (
          SELECT 1 FROM loan_charges c
          WHERE c.loan_id=s.loan_id AND c.schedule_id=s.id AND c.charge_type='Late payment penalty'
        )
      ON CONFLICT (loan_id,schedule_id,charge_type) WHERE charge_type='Late payment penalty' AND schedule_id IS NOT NULL DO NOTHING`);
    await client.query(`UPDATE loan_charges c SET amount=ROUND(s.principal*p.late_penalty_rate/100,2),
        reason='Automatic 5% penalty on the principal due after the due date'
      FROM loan_repayment_schedule s JOIN loans l ON l.id=s.loan_id JOIN loan_products p ON p.id=l.product_id
      WHERE c.schedule_id=s.id AND c.charge_type='Late payment penalty' AND c.status IN ('outstanding','partial')`);
    await client.query("UPDATE legal_contracts SET status='expired',updated_at=NOW() WHERE ends_on<(CURRENT_TIMESTAMP AT TIME ZONE 'Africa/Kampala')::date AND status IN ('approved','legal_approved','active')");
    await client.query("UPDATE investment_contracts SET status='expired' WHERE ends_on<(CURRENT_TIMESTAMP AT TIME ZONE 'Africa/Kampala')::date AND status='active'");
    await client.query(`INSERT INTO notifications (user_id,title,message)
      SELECT u.id,'Loan repayment overdue',
        'Loan '||l.reference||' installment due '||to_char(s.due_date,'DD/MM/YYYY')||' is unpaid. Late penalty of UGX '||
        TRIM(TO_CHAR(ROUND(s.principal*p.late_penalty_rate/100,2),'FM999,999,999,990.00'))||
        ' (5% of the principal due) applies.'
      FROM loan_repayment_schedule s JOIN loans l ON l.id=s.loan_id JOIN loan_products p ON p.id=l.product_id
      JOIN users u ON u.member_id=l.member_id AND u.active=true
      WHERE s.status='overdue' AND s.due_date=(CURRENT_TIMESTAMP AT TIME ZONE 'Africa/Kampala')::date-1
      AND NOT EXISTS (SELECT 1 FROM notifications n WHERE n.user_id=u.id AND n.title='Loan repayment overdue'
        AND n.message LIKE 'Loan '||l.reference||'%' AND n.created_at>=(CURRENT_TIMESTAMP AT TIME ZONE 'Africa/Kampala')::date)`);
    await client.query(`INSERT INTO notifications (user_id,title,message)
