...
async function enforceOneMonthlyWelfareCharge(client) {
  const runner = client || { query: (sql, params) => query(sql, params) };
  const removed = await runner.query(ONE_MONTHLY_WELFARE_SQL);
...
module.exports = {
  loadWelfareStanding,
  loadWelfareMonth,
  enforceOneMonthlyWelfareCharge,
  ONE_MONTHLY_WELFARE_SQL,
  DEFAULT_SINCE,
  VICENT_SINCE,
};
